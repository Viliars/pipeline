...
class TSCNet(nn.Module):
    def __init__(self, num_channel=64, num_features=201, mode='enhancement'):
        ...
        if self.mode == 'auto':
            gate_hidden = max(32, num_channel // 2)
            self.alpha_gate = nn.Sequential(
                nn.AdaptiveAvgPool2d((1,1)),   # (B,C,F,T) -> (B,C,1,1)
                nn.Flatten(),                  # (B,C)
                nn.Linear(num_channel, gate_hidden),
    ...
    def forward(self, x):
        ...
        if self.mode == 'sr':
            ...
            m_sr   = mag + mask
            m_enh  = mag * mask
            alpha  = self.alpha_gate(out_5)           # (B,1)
            alpha  = alpha.view(-1, 1, 1, 1)          # broadcast to (B,1,F,T)
            out_mag = alpha * m_sr + (1 - alpha) * m_enh
